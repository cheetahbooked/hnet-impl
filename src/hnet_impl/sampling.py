...
class ByteTokenizer(Tokenizer):
    ...
    bos_idx: int = 254
    @classmethod
    def construct(cls): return cls()
    def _encode_one(self, s: str) -> TT: return F.pad(
        torch.frombuffer(s.encode(),dtype=torch.uint8),
        (1,0), value=self.bos_idx,
    ).int() if s else torch.tensor([self.bos_idx], dtype=torch.int)
    def encode(self, seqs: list[str]) -> list[TT]:
        with allow_immutable_frombuffer(): return [self._encode_one(s) for s in seqs]
    def decode(self, iids: list[TT]) -> list[str]:
...
def sample_nocache(
    m: nn.Module, iids: TT, bos: int, *,
    max_new: int = 512, temp: float=1.0, min_p: float=0.0001
):
...
def get_termcolor_from_boundaries(bounds: list[TT], cid: int, hid: int) -> list[str]:
    if len(bounds) > 2: raise RuntimeError("this scheme only handles up to stage2")
...
def sample_hnet(m: "HNetLM", iids: TT, bos: int, tok2str_stream: Callable[[TT], TT], **sample_kwargs):
    ...
    for s in tok2str_stream(tok_stream):
        ...
        boundaries = [
            nxt[prv.shape[-1]:] for prv,nxt in zip(b_prev,b_next)
        ]
        attrs,cid,hid = get_termcolor_from_boundaries(boundaries, cid, hid)
        yield s, dict(color=COLOR_CYCLE[cid%2],on_color=HIGHLIGHT_CYCLE[hid%2],attrs=attrs)
...
def completion_sync(p: str, t: Tokenizer, m: "HNetLM", **k) -> str:
    ...
    except UnicodeDecodeError: return colored('[failed to decode UTF-8]', 'red')
    except KeyError: return colored('[failed to decode BPE]', 'red')

def colorize_byte_prefill(p: str, t: ByteTokenizer, m: "HNetLM") -> str:
    iids = t.encode([p])[0][1:].cuda()
    return ''.join(
        colored(c, **k)
        for c,k in colorize_prefill(m, iids, aggregate_bytes_to_utf8)
    )

__all__ = ['ByteTokenizer', 'completion_iter', 'completion_sync', 'colorize_byte_prefill']