...
### helpers ###
def get_stage_cfg(cfg: "AttnConfig | SSMConfig", stage_idx: int) -> dict[str, int]: return {
...
class HNetConfig:
    ...
    def __post_init__(self):
        ...
        if not self.N_compress: self.N_compress[:] = ([1], [1,5], [1,3,9])[len(self.d_model)-1]
    ...
    def lambda_s(self, *, n_gpt: float=4.6):
        return [
            ...
            (n_gpt * n_prod/self.N_compress[-1] * self.d_model[-1]/d) ** .5
    ...
    def load_config(cls, config_path: str, **k) -> 'HNetConfig':
        with open(config_path, "r") as f:
            ...
            if c.get('N_compress', ...) is None: del c['N_compress']
            if k.get('N_compress', ...) is None: del k['N_compress']
    ...
    def create_reasonable_config(cls, D: list[int], arch: list[str], *, d_head: int=64):
        ...
        arch_layout = [arch[-1]]
        for a in reversed(arch[:-1]): arch_layout = [a, arch_layout, a]

        assert all(d%256 == 0 for d in D), "d_model must be divisible by 256"
        def round_to(v: float, n: int=128) -> int: return round(v/n) * n
