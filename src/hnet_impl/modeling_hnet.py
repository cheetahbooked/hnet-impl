...
class STE(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x): return torch.ones_like(x, dtype=torch.float32)
    @staticmethod
...
class QProjPadded(torch.autograd.Function):
    ...
    def backward(ctx, dq_flat: TT):
        ...
        zero_grad = torch.zeros(cu.shape[0]-1, dq_flat.shape[-1], device=dq_flat.device, dtype=dq_flat.dtype)
...
class DeChunkLayer(nn.Module):
    ...
    @staticmethod
    def forward_flat(h_flat: TT, b_flat: TT, p_selected_flat: TT, h_seq_idx: TT, *, eps=1e-4, nheads: int, headdim: int, block_size: int, A: TT):
        ...
        z_bar_flat = mamba_chunk_scan_combined(
            ...
            A, p, c,
            chunk_size=block_size, seq_idx=h_seq_idx,
        )[0].view(-1, h.shape[-1])
    ...
    def forward(self, h_flat: TT, b_flat: TT, p_selected_flat: TT, h_seq_idx: TT, *, eps=1e-4):
        return self.forward_flat(h_flat, b_flat, p_selected_flat, h_seq_idx, eps=eps, nheads=self.nheads, headdim=self.headdim, block_size=self.block_size, A=self.A)
...
class HNet(nn.Module):
    ...
    def block_compile(self, ac: bool):
        ...
        if self.is_innermost: return
        self.encoder.block_compile(ac)
        self.decoder.block_compile(ac)
        self.register_module('routing_module', torch.compile(self.routing_module, backend='inductor',fullgraph=True))
        self.register_module('residual_proj', torch.compile(self.residual_proj, backend='inductor',fullgraph=True))
    ...
    def least_blocking_masked_select(self, *outer_flat_tensors: list[TT], mask_flat: TT, cu_seqlens: TT):
        ...
        for outer in outer_flat_tensors: mutable_res.append(outer.index_select(0, idx_flat))

    def forward(self, x_flat: TT, flat_cu: TT, msl: int):
        d_orig = x_flat.shape[-1]
        x_flat = x_flat if self.pad_dimension is None else torch.cat([x_flat, self.pad_dimension.expand(x_flat.shape[0], -1)], dim=-1)
        x_flat = x_flat.bfloat16()

        if self.is_innermost: return self.main_network(x_flat, flat_cu, msl)[...,:d_orig],[]
        ...
        with self.least_blocking_masked_select(p_flat, r_flat, mask_flat=b_flat, cu_seqlens=select_cu) as (pending_selected_tensors, pending_cpu_stats):
...
class HNetLM(BlockBoundaryMixin, nn.Module):
    ...
    def forward(self, iids: TT, lbls: TT | None = None) -> tuple[TT | tuple[TT,TT], list]:
        ...
        if lbls is None: res = nested.nested_tensor_from_jagged(res, cu_s, max_seqlen=msl)
        return res,extra
    ...
    def split_params_by_hierachy(self) -> list[list[nn.Parameter]]:
        ...
        d = defaultdict(list)
        for n,p in self.named_parameters(): d[n.count('main_network')].append(p)
...
def test_fwd_correctness():
    ...
    with torch.no_grad(): pfill = m(NJT([iids]))[0].values()
    ...
    assert comp == " I hope you are doing well. In this article, we will discuss the basics of the Python programming language. We will start with the basics of Python and then move on to more advanced topics. So, let", comp
