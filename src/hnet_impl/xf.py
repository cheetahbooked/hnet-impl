...
from .torchisms import (
    torch, nn, TT, F, fsdp, dynamo, ptd_checkpoint_wrapper,
    dupe_fn, unsafe_reduce_optimizedmodule_overhead,
)
...
def apply_rotary(
    ...
    fa_rotary.rotary_kernel[grid](
        ...
        seqlen, nheads, seqlen_ro, output.stride(0) if not is_varlen else 0,
        output.stride(-3), output.stride(-2), output.stride(-1),
        x.stride(0) if not is_varlen else 0,
        x.stride(-3), x.stride(-2), x.stride(-1), rotary_dim,
        isinstance(seqlen_offsets, torch.Tensor), is_varlen, interleaved, conjugate,
...
class CausalMHA(nn.Module):
    ...
    def forward(self, x: TT, cu_seqlens: TT, max_seqlen: int):
        ...
        o = flash_attn_varlen_func(*qk.split(self.num_heads,dim=-2),v, cu_seqlens, cu_seqlens, max_seqlen, max_seqlen, causal=True, softmax_scale=self.softmax_scale)
...
class Mamba2Simple(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert d_model * expand // headdim % 8 == 0, "https://github.com/state-spaces/mamba/issues/351#issuecomment-2167091940"          
    ...
    @staticmethod
    def construct_ssm_params(nheads: int, *, A_init_range=(1.,16.), dt_min=1e-3, dt_max=1e-1, dt_init_floor=1e-4):
...
class BlockMeta(type):
    @cache
    @staticmethod
    def make_subclass(cls, arch: str, d: int, h: int, ssm_cfg: frozenset[tuple[str,int]], attn_cfg: frozenset[tuple[str,int]]):
        mixer_cls = dict(
        ...
        namespace = dict(__doc__ = f"Specialised Block: d={d}", d=d, mixer_cls=mixer_cls, mlp_cls=mlp_cls, forward=fake_fwd)
        return type(name, (cls,), namespace)

    def __call__(cls, arch: str, d: int, h: int, ssm_cfg: dict[str,int], attn_cfg: dict[str,int], *args, **kw):
...
class Block(BlockBoundaryMixin, nn.Module, metaclass=BlockMeta):
    ...
    def forward(self, x: TT, residual: TT, cu_seqlens: TT, max_seqlen: int, seq_idx: TT):
        ...
        if isinstance(self.mixer, Mamba2Simple): x = self.mixer(x[None], seq_idx=seq_idx)[0]
        else: x = self.mixer(x, cu_seqlens, max_seqlen)
...
class Isotropic(nn.Module):
    def __init__(self, c: HNetConfig, arch: str, stage_idx: int):
        ...
        assert len(self.first_unique_layer_ids)-1 == len(arch)//2, "there must be exactly one unique code impl per arch"

        # NOTE: set this env to True to speed up sweeping for very small scale models
        self.use_guard_skip = os.environ.get("UNSAFE_GUARD_SKIP",'').lower() in ['true','1']
    ...
    def block_compile(self, ac: bool):
        def apply_ac(l): return ptd_checkpoint_wrapper(l, preserve_rng_state=False) if ac else l
    ...
    def forward(self, x: TT, cu_seqlens: TT, msl: int) -> TT:
        ...
        for i,j in zip(self.first_unique_layer_ids, self.first_unique_layer_ids[1:]):
            ...
            with self.nonfirst_blockctx():
                for l in self.layers[i+1:j]: x, res = l(x,res,cu_seqlens,msl,seq_idx)
...
if __name__ == '__main__':
    ...
    def rand_x(*a): return random_x(*a, device='cuda', dtype=torch.bfloat16, s_min=256, s_max=2048)
    ...
    def short_x(d): return random_x(d, 30, device='cuda', dtype=torch.bfloat16, s_min=4, s_max=16)
    ...
    ## c. do profiling
    make_chrometrace(f'S1-{args_as_str}-m.{enc.use_guard_skip}', dl_0, enc, partial(fwd_block,block=enc))
    make_chrometrace(f'S1-{args_as_str}-T.{net.use_guard_skip}', dl_1, net, partial(fwd_block,block=net))
