...
def unsafe_reduce_optimizedmodule_overhead():
    ...
    with dynamo.set_stance(skip_guard_eval_unsafe=True): yield
...
def random_iids(
    msl: int, *,
    vsize: int=256, s_min: int=256, s_max: int=1024,
...
def random_x(d: int, msl: int, *, device='cuda', dtype=torch.bfloat16, **k):
    for samples in random_iids(msl, **k): yield NJT([
...
def make_chrometrace(name: str, dl: iter, m: nn.Module, fwd_to_loss: callable, *, iters: int=30):
    ...
    with profiler_setup(Path(f'./chrometrace-{name}'), 30) as g:
        for i,inputs in zip(g,dataset):
            ...
            with record_function("fwd"): loss = fwd_to_loss(inputs)
