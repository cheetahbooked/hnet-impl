...
class BlockBoundaryMixin:
    ...
    def is_block(cls, m: nn.Module):
        ...
        if isinstance(m, OptimizedModule): m = m._orig_mod
        if hasattr(m, '_checkpoint_wrapped_module'): m = m._checkpoint_wrapped_module
    ...
    def child_blocks(self):
        ...
        while q:
            if BlockBoundaryMixin.is_block(m := q.popleft()): blocks.append(m)
            else: q += m.children()
        return blocks
