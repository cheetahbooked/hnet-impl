...
def Lin(*a, bias=False, **k): return nn.Linear(*a,bias=bias,**k)
...
class HighPrecLinear(BlockBoundaryMixin, nn.Linear):
    ...
    def apply_fsdp(self, **kw): 
        fsdp.fully_shard(self, **kw|{'mp_policy':fsdp.MixedPrecisionPolicy(param_dtype=torch.float32)})
...
def logsumexp_fwd_kernel(
    x, z, scale,
    D: tl.constexpr, B: tl.constexpr, HAS_SCALE: tl.constexpr
):
...
def cross_entropy_kernel(
    logits, lse, target, loss, ignore_index,
    label_smoothing: tl.constexpr, logit_scale: tl.constexpr, V: tl.constexpr, BV: tl.constexpr
):
...
def fused_linear_cross_entropy_forward(
    ...
    numel_scale = 1./N
    scale = logit_scale * numel_scale # instead of passing logit_scale to kernel, we scale in mm for numerical stability
    ...
    for ic in range(NC):
        ...
        cross_entropy_kernel[(c_logits.shape[0],)](
            logits=c_logits, lse=c_lse, target=c_target, loss=c_loss, ignore_index=ignore_index,
            label_smoothing=label_smoothing, logit_scale=1.0, V=V, BV=BV, num_warps=STATIC_WARPS,
        )
        ...
        if weight is not None: dw += torch.empty_like(dw,dtype=c_x.dtype).addmm_(c_logits.mT, c_x, beta=0, alpha=scale)
        if bias is not None: db.add_(c_logits.sum(0), alpha=scale)

    if dw is not None: dw = dw.to(weight)
    if db is not None: db = db.to(bias)
    loss_sum = loss.sum()
...
class FusedLinearCrossEntropyFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, x: torch.Tensor, target: torch.LongTensor, weight: torch.Tensor, bias: torch.Tensor = None,
        logit_scale: float = 1.0, ignore_index: int = -100, label_smoothing: float = 0.0, num_chunks: int = 8,
    ):
        ...
        loss_mean, loss_sum, dx, dw, db = fused_linear_cross_entropy_forward(
            ...
            ignore_index, label_smoothing, logit_scale, num_chunks,
        )
        ...
        return loss_mean, loss_sum # <-- sum should only be used for metric calculation (bpb)
...
class LMHead(nn.Linear):
    ...
    def forward(self, x: TT, labels: TT | None = None) -> TT:
        if labels is None: return F.linear(x, self.weight) * self.logit_scale
